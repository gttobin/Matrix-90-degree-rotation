...
class Bitmap
{
  ...
  public:
    ...
    **/
    ...
    unsigned char**  toGreyByteMatrix();
    int* toIntMatrix();

    void saveByteMatrix(byte** grey,std::string filename);

    void bytesToPixels(byte** img,int height,int width);
    void intToPixels(int* img,int height,int width);
    void bytesToPixelsSquare(byte** ,int);
    byte** crop(byte** input,Rectangle crop,int height,int width);
    void createEmptyBitmap(std::string file_name,int height,int width);
